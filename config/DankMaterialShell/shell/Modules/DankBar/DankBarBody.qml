...
Item {
    ...
    function _updateShouldHideForWindows() {
        ...
        if (CompositorService.isHyprland) {
            // Reuses the dock's own generic overlap check (position/thickness are
            // plain parameters there, not dock-specific) so tiled, maximized and
            // fullscreen windows are all handled by actual geometry, and a
            // floating window that never reaches the bar band leaves it revealed.
            const barThickness = barWindow.effectiveBarThickness + (barConfig?.spacing ?? 4);
            shouldHideForWindows = CompositorService.hyprlandDockOverlapForSmartAutoHide(screenName, barConfig?.position ?? 0, barThickness, barWindow.screen?.width ?? 0, barWindow.screen?.height ?? 0);
            return;
        }
    ...
    Item {
        ...
        property bool reveal: {
            // Screen frame/border is a deliberate, always-on chrome choice; it takes
            // priority over dodge-windows and plain autoHide alike.
        ...
        // Edge-only hover reveal: while hidden, topBarMouseArea still spans
        // the full bar+gap band (its geometry doesn't follow topBarSlide),
        // so any entry into that band would otherwise reveal the bar. Reuse
        // the existing click-through gap suppression for every auto-hide
        // mode, and require a short dwell inside inEdgeStrip() before
        // actually lifting the suppression.
        Timer {
            id: edgeHoverRevealTimer
            interval: 500
            repeat: false
            onTriggered: {
        ...
        Connections {
            ...
            function onContainsMouseChanged() {
                if (!topBarMouseArea.containsMouse) {
                    topBarCore.gapEnterSuppressed = false;
                    edgeHoverRevealTimer.stop();
                } else if (!topBarCore.reveal) {
        ...
        MouseArea {
            ...
            onPositionChanged: mouse => {
                ...
                if (!edgeHoverRevealTimer.running)
                    edgeHoverRevealTimer.start();
            }
